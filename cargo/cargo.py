...
class Cargo:
    ...
    def __init__(
        self,
        use_dgi: bool = False,
        json_sdg_path: str = None,
        transactions_json: str = None,
        dgi_neo4j_hostname: str = "localhost",
        dgi_neo4j_hostport: int = 7687,
        ...
        if use_dgi:
            ...
            self.graph = self.json2nx(json_sdg_path, transactions_json)

        Cargo.verbose = verbose

    def json2nx(self, path_to_sdg_json: str, transactions_json: str) -> nx.MultiDiGraph:
        ...
        for key, group in itertools.groupby(
            self.json_graph["nodes"], key=lambda x: x["id"]
        ):
    ...
    def dgi2networkx(
        self, neo4j_url: str, dgi_neo4j_uname: str, dgi_neo4j_passw: str
    ) -> nx.MultiDiGraph:
    ...
    def assign_init_labels(self, G, init_labels, max_part, labels_file):
        ...
        if init_labels == "auto":
            ...
            if (len(partitions) / float(len(G.nodes))) <= 0.10:
                raise Exception(
                    "Only {} out of {} nodes were initialized from the file".format(
    ...
    def do_cargo(self, init_labels="auto", max_part=None, labels_file=None):
        ...
        partition_freqs = np.array(
            np.unique(
                list(nx.get_node_attributes(labelprop_G, "partition").values()),
                return_counts=True,
            )
        )
        partition_freqs = np.array(partition_freqs).T
        partition_freqs = {
            partition: freq for partition, freq in partition_freqs if partition != -1
        }
    ...
    def get_metrics_from_file(self, labels_file):
        ...
        self.assign_init_labels(
            new_G, init_labels="file", max_part=None, labels_file=labels_file
        )
    ...
    def run(
        ...
        nx.set_node_attributes(
            labelprop_G, nx.degree_centrality(labelprop_G), "data_centrality"
        )
        ...
        class_graph_view = TransformGraph.from_method_graph_to_class_graph(
            method_sdg_as_dict=method_graph_view,
            transactions_as_dict=self.transactions_json,
        )

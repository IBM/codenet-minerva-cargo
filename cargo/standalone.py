...
@click.option('--app-dependency-graph-path', '-i', type=click.Path(exists=True), 
...
@click.option('--sdg-filename', '-f', type=str, default='sdg.json', 
              help='Filename JSON file. If you used https://github.com/konveyor/dgi-code-analyzer '
              'to get the system dependency graph of an application, then the default filename is sdg.json')
...
def minerva_cargo(max_partitions, app_dependency_graph_path, sdg_filename, seed_partitions, output):
    """
    ...
    cargo = Cargo(json_sdg_path=Path(app_dependency_graph_path).joinpath(sdg_filename))
